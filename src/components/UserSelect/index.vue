...
<template>
  <PqTableSelect
    ...
    popperClass="pq-user-select-modal pq-modal-show-header-border pq-modal-show-footer-border"
    :popperProps="{ flexTable: 'auto', ...(popperProps || {}) }"
    :selectStyle="isPickerMode ? 'none' : selectStyle"
    selectClass="pq-user-select"
  ...
  >
    ...
    >
      <div
        ...
        class="pq-user-select-item"
        :class="{ 'is-icon': !item.data?.[avatarField] }"
      >
        <div
          class="pq-user-select-item-avatar"
          :class="{ 'is-icon': !item.data?.[avatarField] }"
        >
          <img
          ...
          <ElIcon
            class="pq-user-select-item-del"
            @click="multiple ? removeItem(item) : clear()"
          >
            <CloseOutlined />
          </ElIcon>
        </div>
        <div class="pq-user-select-item-name">{{ item.label }}</div>
      </div>
      <div
        v-if="!(!multiple && selected.length)"
        class="pq-user-select-item is-icon-add"
        @click="openPopper()"
      >
        <ElIcon class="pq-user-select-item-avatar">
...
<script setup>
  ...
  import { CloseOutlined, PlusOutlined } from 'pq-admin-ent/es/icons';
  import { mapTree } from 'pq-admin-ent';
  import { omit } from 'pq-admin-ent/es/utils/common';
  import { useComponentEvents } from 'pq-admin-ent/es/utils/hook';
  import {
    tableSelectProps,
    tableSelectEmits
  } from 'pq-admin-ent/es/pq-table-select/props';
...
<style lang="scss">
  @use 'pq-admin-ent/es/style/util.scss' as *;
  @use 'pq-admin-ent/es/pq-pro-table/style/css-var.scss' as *;
  ...
  .pq-user-select {
    ...
    .pq-user-select-item.is-icon-add > .pq-user-select-item-avatar {
      font-size: 19px;
  ...
  /* 弹框 */
  .pq-user-select-modal {
    .pq-modal-body,
    .pq-drawer-body {
      padding: 0;
    }

    &.pq-table-select-popper .pq-popover-body {
      padding: 0;

      & > .pq-user-select-wrapper {
        height: 100%;
      }
    }
  }
  ...
  /* 折叠面板 */
  .pq-user-select-wrapper.pq-split-panel {
    & > .pq-split-panel-body {
      flex-direction: row;
    }

    & > .pq-split-panel-wrap > .pq-split-panel-side {
      border-width: 0 1px 0 0;
    }
  }

  /* 树 */
  .pq-user-select-tree-wrapper {
    flex: 1 1 60px;
    padding: 0 0 10px 0;
    box-sizing: border-box;
    overflow: auto;
    ...
    .pq-user-select-tree {
      padding: 0 calc(pqVar('tree', 'item-radius') / 2 * 5);
      #{pqVarName('tree', 'item-height')}: 34px;
    }

    .pq-user-select-tree-label > .el-icon {
      margin-right: 4px;
      vertical-align: -2px;
    }
  }

  .pq-user-select-tree-search {
    flex-shrink: 0;
  ...
  .pq-user-select-body {
    ...
    & > .pq-pro-table {
      ...
      & > .pq-pro-table-view {
        #{pqVarName('table', 'th-bg')}: elVar('fill-color', 'light');
        #{elVarName('bg-color')}: elVar('bg-color', 'overlay');
      }

      & > .pq-pro-table-footer > .pq-pagination {
        padding-top: 0;
      }
  ...
  .pq-user-select-right-body {
    ...
    &.is-sortable .pq-user-select-right-item {
      cursor: move;
    }
  }

  /* 选中列表项 */
  .pq-user-select-right-item {
    flex-shrink: 0;
    display: flex;
    align-items: center;
    padding: 4px;
  ...
  @media screen and (max-width: 767.99px) {
    .pq-user-select-body > .pq-pro-table {
      padding-bottom: 8px;
    }

    .pq-user-select-wrapper.pq-split-panel > .pq-split-panel-body {
      flex-direction: column;
    }
