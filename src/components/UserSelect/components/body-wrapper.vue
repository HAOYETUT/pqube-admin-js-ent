<template>
  <PqSplitPanel
    ...
    class="pq-user-select-wrapper"
  >
    <div class="pq-user-select-tree-search">
      <ElInput
        :clearable="true"
    ...
    <PqLoading :loading="treeLoading" class="pq-user-select-tree-wrapper">
      <ElTree
        ref="treeRef"
        :data="treeData"
        :props="treeProps"
        ...
        class="pq-user-select-tree"
        @nodeClick="handleTreeItemClick"
      >
        <template #default="{ node }">
          <span
            class="el-tree-node__label pq-user-select-tree-label"
            :title="node.label"
          >
            <ElIcon>
    ...
    <template #body>
      <div class="pq-user-select-body">
        <!-- 搜索表单 -->
        <ElForm
          labelWidth="78px"
          class="pq-user-select-search"
      ...
      <!-- 已选列表 -->
      <div class="pq-user-select-right">
        <div class="pq-user-select-right-header">
          <div class="pq-user-select-right-title">
          ...
          <ElLink
            type="danger"
            underline="never"
            class="pq-user-select-right-clear"
        ...
        <VueDraggable
          ...
          :delay="150"
          :disabled="!multiple"
          class="pq-user-select-right-body"
          :class="{ 'is-sortable': multiple }"
          :modelValue="selectedItems"
          @update:modelValue="updateModelValue"
        >
          <template #item="{ element: item }">
            <div class="pq-user-select-right-item">
              <ElIcon v-if="itemIcon" class="pq-user-select-right-icon">
                <component :is="itemIcon" />
              </ElIcon>
              <div
                v-else-if="item.data?.[avatarField]"
                class="pq-user-select-right-avatar"
              >
                <img :src="item.data?.[avatarField]" />
              </div>
              <div v-else class="pq-user-select-right-avatar is-icon">
                {{
              ...
              <div class="pq-user-select-right-label">{{ item.label }}</div>
              <ElIcon
                class="pq-user-select-right-del"
