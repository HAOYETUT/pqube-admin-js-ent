<template>
  <pq-page>
      ...
      <el-row>
        <el-col :sm="8" :xs="24">
          <div style="text-align: center">
            <pq-text type="placeholder">进行中的任务</pq-text>
            <pq-text size="xl" style="margin-top: 6px">10 个任务</pq-text>
          </div>
        </el-col>
        <el-col :sm="8" :xs="24">
          <div style="text-align: center">
            <pq-text type="placeholder">剩余任务</pq-text>
            <pq-text size="xl" style="margin-top: 6px">3 个任务</pq-text>
          </div>
        </el-col>
        <el-col :sm="8" :xs="24">
          <div style="text-align: center">
            <pq-text type="placeholder">任务总耗时</pq-text>
            <pq-text size="xl" style="margin-top: 6px">120 个小时</pq-text>
          </div>
        </el-col>
      </el-row>
    </pq-card>
    <pq-card>
      <!-- 头部工具栏 -->
      <pq-toolbar title="复杂列表" :title-props="{ size: 'md' }">
        <template #tools>
      ...
      <div style="margin-top: 8px">
        <el-button :icon="PlusOutlined" class="pq-fluid" @click="openEdit()">
          ...
          <div class="list-item">
            <div class="list-item-avatar">
              ...
              <div class="list-item-avatar-extra">
                ...
                <pq-text size="sm" type="placeholder">
                  {{ item.content }}
                </pq-text>
              </div>
            </div>
            <div class="list-item-owner">
              ...
              <pq-text size="sm" type="placeholder">{{ item.user }}</pq-text>
            </div>
            <div class="list-item-time">
              <div class="list-item-title">开始时间</div>
              <pq-text size="sm" type="placeholder">{{ item.time }}</pq-text>
            </div>
      ...
      data.value = [
        {
          ...
          progress: 87,
          content:
            'Pqment, 一套为开发者、设计师和产品经理准备的基于 Vue 2.0 的组件库, 提供了配套设计资源, 帮助你的网站快速成型。',
          cover:
  ...
  const handleSubmit = () => {
    formRef.value?.validate?.((valid) => {
      ...
      setTimeout(() => {
        ...
        PqMessage.success({ message: '保存成功', plain: true });
        if (form.id) {
          // 保存修改
  ...
  const dropClick = (key, item) => {
    if (key === 'remove') {
      ...
      ElMessageBox.confirm(`确定删除“${item.title}”吗?`, '系统提示', {
        ...
        .then(() => {
          PqMessage.success({ message: '点击了删除', plain: true });
        })
        .catch(() => {});
    } else if (key === 'share') {
      PqMessage.success({ message: '点击了分享', plain: true });
    }
  };
