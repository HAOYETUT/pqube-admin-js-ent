<template>
  <pq-page :flex-table="fixedHeight">
    <!-- 搜索表单 -->
    ...
    >
      ...
      >
        <template #title>
          <span>已选择&nbsp;</span>
          <pq-text strong type="primary" tag="span">
            {{ selections.length }}
          </pq-text>
          <span>&nbsp;项数据&emsp;</span>
      ...
      >
        <template #toolbar>
          ...
          >
            <el-button :disabled="!selections.length" class="pq-btn-icon">
              <span>批量操作</span>
        ...
        <!-- 状态列 -->
        <template #status="{ row }">
          <pq-dot v-if="row.status === 0" text="正常" size="8px" />
          <pq-dot
...
<script setup>
  ...
  import { getExportWorkbook } from 'pq-admin-ent/es/pq-pro-table/exceljs-plugin';
  ...
  const handleDropClick = (command) => {
    if (command === 'del') {
      PqMessage.error({ message: '点击了批量删除', plain: true });
    } else if (command === 'edit') {
      PqMessage.info({ message: '点击了批量修改', plain: true });
    } else if (command === 'disabled') {
      PqMessage.info({ message: '点击了批量禁用', plain: true });
    }
  };
