<template>
  <pq-page flex-table="auto" hide-footer :multi-card="false">
    <pq-card flex-table="auto" :body-style="{ padding: 0 }">
      <pq-split-panel
        flex-table="auto"
        :space="0"
        :size="258"
      ...
      >
        ...
        >
          <el-tree
            ...
            :style="{
              '--pq-tree-item-height': '34px',
              '--pq-tree-expand-margin':
                '0 2px 0 calc(8px - var(--pq-tree-item-radius))',
              padding: '0 calc(var(--pq-tree-item-radius) * 3)'
            }"
...
<script setup>
  ...
  import { PqMessage, toTree, queryChild, useModal } from 'pq-admin-ent';
  import {
    PlusOutlined,
    EditOutlined,
    DeleteOutlined,
  ...
  const removeTree = (item) => {
    ...
    ElMessageBox.confirm(`确定要删除“${row.organizationName}”吗?`, '系统提示', {
      ...
      .then(() => {
        const loading = PqMessage.loading({
          message: '请求中..',
          plain: true
        });
        removeOrganization(row.organizationId)
          .then((msg) => {
            loading.close();
            PqMessage.success({ message: msg, plain: true });
            query();
          })
          .catch((e) => {
            loading.close();
  ...
  const remove = (row) => {
    ...
    if (!rows.length) {
      PqMessage.error({ message: '请至少选择一条数据', plain: true });
      return;
    }
