<template>
  <pq-card header="更多示例">
    ...
    <pq-printer v-model="printing1" target="_iframe">
      <div style="font-size: 22px; margin-bottom: 24px; line-height: 1.2">
        表格打印示例：
      </div>
      <pq-table size="large" :border="true" style="page-break-after: always">
        <colgroup>
          <col width="60px" />
          <col width="200px" />
          <col width="120px" />
        ...
        <tbody>
          <tr v-for="(row, index) in projects" :key="row.id">
            ...
            <td>{{ row.endDate }}</td>
            <td>
              <pq-text v-if="row.status === 0" type="success">进行中</pq-text>
              <pq-text v-else-if="row.status === 1" type="danger">
                已延期
              </pq-text>
              <pq-text v-else-if="row.status === 2" type="warning">
                未开始
              </pq-text>
              <pq-text v-else-if="row.status === 3" type="info">
                已结束
              </pq-text>
            </td>
          </tr>
      ...
      <pq-table
        ...
        :style="{
          ...
          '--pq-table-bg': '#fff',
          '--pq-table-color': '#000',
          '--pq-table-border-color': '#000',
          '--pq-table-th-color': '#000',
          '--pq-table-th-font-weight': 'bold',
          '--pq-table-th-bg': 'transparent',
          '--pq-table-tr-bg': 'transparent',
          '--pq-table-tr-hover-bg': 'transparent',
          '--pq-table-radius': 0,
          '--pq-table-sm-radius': 0,
          '--pq-table-lg-radius': 0,
        ...
        <pq-table>
          ...
          <tbody>
            <tr v-for="(row, index) in projects" :key="row.id">
              ...
              <td>{{ row.endDate }}</td>
              <td>
                <pq-text v-if="row.status === 0" type="success">
                  进行中
                </pq-text>
                <pq-text v-else-if="row.status === 1" type="danger">
                  已延期
    ...
    <pq-printer v-model="printing2" target="_iframe">
      <div style="font-size: 22px; margin-bottom: 24px; line-height: 1.2">
        图片打印示例：
      </div>
      <img
    ...
    <pq-printer v-model="printing3" target="_iframe">
      <div style="font-size: 22px; margin-bottom: 24px; line-height: 1.2">
        二维码打印示例：
      </div>
      <div
      ...
      >
        <pq-qr-code-svg :value="partyA" :size="180" />
        <div style="font-size: 14px; line-height: 1.2">{{ partyA }}</div>
      </div>
    </pq-printer>
    <pq-printer v-model="printing4" target="_iframe">
      <div style="font-size: 22px; margin-bottom: 24px; line-height: 1.2">
        水印组件：
      </div>
      <pq-watermark
    ...
    <pq-printer v-model="printing5" target="_iframe">
      <div style="font-size: 22px; margin-bottom: 24px; line-height: 1.2">
        echarts图表打印示例：
      </div>
      <v-chart
...
<script setup>
  ...
  import { ChartTheme } from 'pq-admin-ent';
