<template>
  <pq-page :multi-card="false">
    <pq-card :body-style="{ padding: 0, overflow: 'hidden' }">
      <pq-split-panel
        space="0px"
        size="180px"
        ...
        style="min-height: 708px; border-radius: var(--pq-card-radius)"
      >
        <pq-menus :items="items" :default-active="active" class="demo-menu" />
        <template #body>
...
<script setup>
  ...
  watch(
    () => route.query.type,
    (type) => {
      if (route.path === '/extension/table') {
        active.value = [type].flat()[0] || 'base';
        // 移动端自动收起左侧
        if (mobile.value) {
          collapse.value = true;
        }
