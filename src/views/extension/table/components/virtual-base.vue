<template>
  <div>
    ...
    >
      <template #toolbar>
        <el-button type="primary" class="pq-btn-icon" @click="getSelections">
          获取多选行数据
        </el-button>
        <el-button
...
<script setup>
  ...
  /** 表格数据源 */
  const datasource = async ({ pages, filter, orders }) => {
    ...
    if (orders != null && orders.sort) {
      list.sort((a, b) => {
        const aValue = a[orders.sort];
        const bValue = b[orders.sort];
        if (aValue == bValue) {
          return 0;
        }
  ...
  /** 打开编辑弹窗 */
  const openEdit = (row) => {
    PqMessage.success({ message: `编辑: ${row.nickname}`, plain: true });
  };

  /** 删除单个 */
  const remove = (row) => {
    PqMessage.error({ message: `删除: ${row.nickname}`, plain: true });
  };
  ...
  /** 导出和打印全部数据的数据源 */
  const exportSource = async ({ filter, orders }) => {
