...
<script setup>
  ...
  /** 打开右键菜单 */
  const openCtxMenuDropdown = (triggerEl, row) => {
  ...
  const handleItemCommand = (command) => {
    ...
    if (command === 'psw') {
      ...
      PqMessage.info({ message: `点击了修改: ${row.nickname}`, plain: true });
    } else if (command === 'del') {
      PqMessage.info({ message: `点击了删除: ${row.nickname}`, plain: true });
  ...
  /** 表格行右键事件 */
  const handleRowContextmenu = (row, _column, e) => {
    e.preventDefault();
    e.stopPropagation();
