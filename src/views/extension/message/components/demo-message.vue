<template>
  <pq-card header="消息提示">
    ...
    <option-item label="合并相同内容">
      <el-button @click="showGroup('success', '内容一')" class="pq-btn-icon">
        内容一
      </el-button>
      <el-button @click="showGroup('success', '内容二')" class="pq-btn-icon">
        内容二
      </el-button>
      <el-button @click="showGroup('warning', '内容一')">警告</el-button>
      <el-button @click="showGroup('error', '内容一')">错误</el-button>
    </option-item>
    <option-item label="限制在指定区域">
      <el-button @click="showInDemoWrap" class="pq-btn-icon">提示</el-button>
      <el-button @click="openModal" class="pq-btn-icon">
        限制在指定弹窗
      </el-button>
    </option-item>
    <div
    ...
    >
      ...
      <div ref="demoMessageWrapRef" class="pq-message-wrapper"></div>
    </div>
...
<script setup>
  import { ref } from 'vue';
  import { useMessage, useMessageBox, random } from 'pq-admin-ent';
  ...
  const showReactiveLoading = () => {
    ...
    const loading = message.loading({
      message: () =>
        `正在导入用户......${String(progress.value).padStart(2, '0')}%`,
      plain: true,
      inner: inner.value,
      mask: true,
      centered: true
