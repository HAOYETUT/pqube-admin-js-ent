<template>
  <pq-card header="Popover、PopConfirm、Dropdown">
    <div>
      <el-space wrap style="margin-bottom: 20px">
        ...
        >
          <template #reference>
            <el-button class="pq-btn-icon">Popover</el-button>
          </template>
        ...
        >
          <template #reference>
            <el-button class="pq-btn-icon">PopConfirm</el-button>
          </template>
        </pq-popconfirm>
        <pq-popconfirm
          :width="258"
        ...
        >
          <template #reference>
            <el-button class="pq-btn-icon">PopConfirm 自定义按钮</el-button>
          </template>
    ...
    <div>
      <el-space wrap>
        ...
        <pq-dropdown
          ...
          trigger="click"
          :split-button="true"
          :split-button-props="{ class: 'pq-btn-icon' }"
          @command="handleCommand"
          @click="handleClick"
        >
          <span>按钮组</span>
...
<script setup>
  ...
  const handleCommand = (command) => {
    PqMessage.info({ message: `点击了: ${command}`, plain: true });
  };

  const handleClick = () => {
    PqMessage.info({ message: '点击了按钮组', plain: true });
  };

  const handleConfirm = () => {
    PqMessage.info({ message: '点击了确定', plain: true });
  };

  const handleAgree = () => {
    PqMessage.info({ message: '点击了满意', plain: true });
  };

  const handleComplain = (cancel) => {
    cancel && cancel();
    PqMessage.info({ message: '点击了投诉', plain: true });
  };
</script>
