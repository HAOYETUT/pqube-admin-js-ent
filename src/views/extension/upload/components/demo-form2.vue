...
<script setup>
    ...
    idCardImg1: [
      ...
      },
      {
        ...
        validator: (_rule, value, callback) => {
          if (value && value.some((d) => d.status !== 'done')) {
  ...
  const save = () => {
    formRef.value?.validate?.((valid) => {
      ...
      PqMessage.success({ message: '数据已打印在控制台', plain: true });
    });
  };
  ...
  const checkFile = (file) => {
    ...
    if (!file.type.startsWith('image')) {
      PqMessage.error({ message: '只能选择图片', plain: true });
      return;
    }
    if (file.size / 1024 / 1024 > 2) {
      PqMessage.error({ message: '大小不能超过 2MB', plain: true });
      return;
    }
    return true;
  };
  ...
  /** 上传事件 */
  const handleUpload = (type, data, retry) => {
  ...
  /** 修改事件 */
  const handleEditUpload = (type, { item, newItem }) => {
