<template>
  <pq-modal :width="460" title="移动到" v-model="visible" @open="handleOpen">
    <div class="demo-file-move-tree">
      ...
      >
        <template #default="scope">
          <img
            src="/pq-file-list/ic_file_folder.png"
...
<script setup>
  import { ref } from 'vue';
  import { PqMessage, toTree, mapTree } from 'pq-admin-ent';
  ...
  const save = () => {
    ...
    if (parentId == null) {
      PqMessage.error({ message: '请选择分组', plain: true });
      return;
    }
    loading.value = true;
...
<style lang="scss" scoped>
  .demo-file-move-tree {
    ...
    :deep(.el-tree) {
      padding-top: 8px;
      padding-bottom: 8px;
      padding-left: calc(var(--pq-tree-item-radius) * 2);
      padding-right: calc(var(--pq-tree-item-radius) * 2);
      box-sizing: border-box;
      overflow-x: hidden;
      overflow-y: auto;
      user-select: none;
      --pq-tree-item-height: 36px;
      ...
      .el-tree-node__content {
        ...
        & > .el-tree-node__label.is-active {
          ...
          &::before {
            ...
            border-radius: var(--pq-tree-item-radius);
          }
        }
