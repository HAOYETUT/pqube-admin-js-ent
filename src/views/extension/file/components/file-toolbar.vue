<template>
  <pq-toolbar theme="plain" class="demo-file-toolbar">
    <el-upload
      action=""
    ...
    >
      <el-button type="primary" class="pq-btn-icon" :icon="UploadOutlined">
        上传
      </el-button>
    </el-upload>
    <el-button
    ...
    <el-button
      ...
      class="pq-btn-icon hidden-xs-only"
      @click="removeBatch"
    >
      删除
    </el-button>
...
<script setup>
  ...
  const handleUpload = (file) => {
    if (file.size / 1024 / 1024 > 100) {
      PqMessage.error({ message: '大小不能超过 100MB', plain: true });
      return false;
    }
    ...
    uploadFile(file)
      .then((data) => {
        addUserFile({
          ...
          .then(() => {
            loading.close();
            PqMessage.success({ message: '上传成功', plain: true });
            handleDone();
          })
          .catch((e) => {
            loading.close();
