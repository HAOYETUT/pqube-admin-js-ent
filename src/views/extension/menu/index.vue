    ...
    <pq-card header="垂直菜单">
      ...
      <div style="display: flex; flex-wrap: wrap">
        <div style="margin: 0 12px 12px 0">
          <el-button type="primary" class="pq-btn-icon" @click="changeData()">
            一级菜单分组形式
          </el-button>
        </div>
        <div style="margin: 0 12px 12px 0">
          <el-button type="primary" class="pq-btn-icon" @click="changeColor()">
            更改菜单图标颜色
          </el-button>
        </div>
      </div>
...
<script setup>
  ...
  /** 垂直菜单数据 */
  const menuItems = ref(getMenuData());

  /** 水平菜单数据 */
  const navItems = ref(getMenuData());

  /** 紧凑菜单数据 */
  const boxItems = ref(getMenuData());

  /** 一级菜单改为分组形式 */
  const changeData = (type) => {
    const result = getMenuData().map((d) => {
      const t = { ...d };
      if (t.children) {
        t.group = true;
        t.icon = void 0;
...
<style lang="scss">
  .demo-menu-popper.pq-menu > .el-menu--popup-container > .el-menu--popup {
    overflow: visible;
