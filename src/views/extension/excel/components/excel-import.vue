<template>
  <pq-card header="导入 Excel">
    <el-space>
      ...
      >
        <el-button type="primary" class="pq-btn-icon">导入</el-button>
      </el-upload>
      <el-upload
        action=""
        accept=".xls,.xlsx"
      ...
      >
        <el-button type="primary" class="pq-btn-icon">
          导入拆分合并
        </el-button>
      </el-upload>
      <el-upload
...
<script setup>
  ...
  const importFile = (file) => {
    if (
      ...
      PqMessage.error({ message: '只能选择 excel 文件', plain: true });
      return false;
    }
    if (file.size / 1024 / 1024 > 20) {
      PqMessage.error({ message: '大小不能超过 20MB', plain: true });
      return false;
    }
    const loading = PqMessage.loading({ message: '加载中..', plain: true });
