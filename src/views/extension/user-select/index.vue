<template>
  <pq-page>
    ...
    <pq-card header="自定义数据">
      <div style="margin: 0 0 18px 0">
        用户选择组件基于表格选择组件开发，支持 PqTableSelect
...
<script setup>
  ...
  const beforeConfirm = (result) => {
    if (!result) {
      PqMessage.error({ message: '请选择一个用户', plain: true });
      return false;
    }
  };
  ...
  const beforeConfirm2 = (result) => {
    if (!result?.length) {
      PqMessage.error({ message: '请至少选择一个用户', plain: true });
      return false;
    }
  };
