<template>
  <pq-card header="基础用法" :body-style="{ padding: '12px' }">
    <frame-card :style="mobile ? { maxWidth: '420px' } : {}">
      ...
      >
        <pq-admin-layout
          ...
          :style="{
            '--pq-header-height': headerHeight + 'px',
            '--pq-sidebar-width': sidebarWidth + 'px',
            '--pq-sidebox-width': sideboxWidth + 'px',
            '--pq-sidebar-mix-width': sidebarMixWidth + 'px'
          }"
        >
          <template #logo>
...
<script setup>
  ...
  import { LayoutTool } from 'pq-admin-ent';
  import { useTimer } from 'pq-admin-ent/es/utils/hook';
  import {
    SettingOutlined,
    UserOutlined,
    IdcardOutlined,
  ...
  const sidebarLayout = ref('mix');

  const tabBar = ref(true);

  const breadcrumb = ref(true);

  const headerStyle = ref('light');

  const sidebarStyle = ref('light');

  const mixSidebarStyle = ref('light');

  const tabStyle = ref('button');
