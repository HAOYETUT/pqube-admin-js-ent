<template>
  <pq-admin-layout
    ...
    :style="{
      background: 'none',
      '--pq-header-tool-padding': mobile ? '8px' : '10px'
    }"
  >
    <template #logo>
    ...
    <div :style="{ flex: 'auto' }">
      <el-carousel
        height="320px"
        style="margin-top: calc(0px - var(--pq-header-height) - 1px)"
      >
      ...
      >
        <div style="text-align: center; padding: 28px 0 20px 0">
          <pq-text size="lg" type="heading">标题标题标题</pq-text>
          <pq-text size="sm" type="secondary">描述描述描述</pq-text>
        </div>
        <el-row :gutter="16">
          <el-col v-for="index in 8" :key="index" :md="6" :sm="8" :xs="12">
            <pq-card style="border-radius: 0; min-height: 140px" />
          </el-col>
        </el-row>
      </div>
      <div
      ...
      return [
        {
          ...
          children: menus,
          style: {
            '--pq-menu-arrow-size': props.mobile ? '0px' : '14px',
            '--pq-menu-horizontal-active-bg': 'transparent',
            '--pq-menu-horizontal-line-size': 0,
