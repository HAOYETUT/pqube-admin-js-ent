<template>
  <pq-page class="workplace-page">
    ...
    <pq-card :body-style="{ padding: 0 }" style="overflow: hidden">
    ...
    <!-- 添加卡片弹窗 -->
    <pq-modal :width="680" v-model="visible" title="未添加的卡片">
...
<script setup>
  ...
  const getCacheData = () => {
    ...
    try {
      const temp = cache ? JSON.parse(cache) : null;
      if (temp) {
  ...
  const reset = () => {
    ...
    PqMessage.success({ message: '已重置', plain: true });
  };
  ...
  const handleCommand = (command, index) => {
    switch (command) {
      case 'refresh': // 刷新
        PqMessage.info({ message: '点击了刷新', plain: true });
        break;
      case 'edit': // 编辑
        PqMessage.info({ message: '点击了编辑', plain: true });
        break;
      case 'remove': // 删除
  ...
  const addView = (item) => {
    ...
    cacheData();
    PqMessage.success({ message: '已添加', plain: true });
  };

  onMounted(() => {
    // 卡片支持拖动排序
    sortableIns = new SortableJs(wrapRef.value?.$el, {
      handle: '.pq-card-header',
