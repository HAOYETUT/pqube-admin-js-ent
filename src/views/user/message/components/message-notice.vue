<template>
  <div>
    ...
    >
      <template #toolbar>
        <el-button type="primary" class="pq-btn-icon" @click="confirm()">
          批量确认
        </el-button>
        <el-button type="danger" class="pq-btn-icon" @click="remove()">
          删除通知
        </el-button>
      </template>
      <template #status="{ row }">
        <pq-text v-if="row.status === 0" type="warning">未确认</pq-text>
        <pq-text v-else-if="row.status === 1" type="info">已确认</pq-text>
      </template>
      <template #action="{ row }">
...
<script setup>
  import { ref } from 'vue';
  import { useMessage } from 'pq-admin-ent';
