<template>
  <pq-page plain :multi-card="false">
    <pq-card style="border-radius: 0">
      <pq-text type="heading" size="lg">复杂表单</pq-text>
    ...
    <pq-page>
      ...
      >
        ...
        <pq-card header="选择成员">
          ...
          >
            <pq-table style="min-width: 580px; table-layout: fixed">
              <colgroup>
                <col width="60px" />
                <col />
                <col />
              ...
              <tbody style="--pq-table-padding: 6px 8px">
                ...
                <tr v-if="!form.users || !form.users.length">
                  <td colspan="5" style="text-align: center">
                    <pq-text style="padding: 4px 0" type="secondary">
                      暂无数据
                    </pq-text>
                  </td>
                </tr>
    ...
    <!-- 底部工具栏 -->
    <pq-bottom-bar teleported>
      <pq-text v-if="validMsg" type="danger" :icon="CloseCircleOutlined">
        <span>{{ validMsg }}</span>
      </pq-text>
      <template #extra>
...
<script setup>
  ...
  const submit = () => {
    formRef.value?.validate?.((valid, obj) => {
      ...
      setTimeout(() => {
        loading.value = false;
        PqMessage.success({ message: '提交成功', plain: true });
        resetFields();
      }, 1000);
    });
  };
  ...
  /** 选择成员表格容器 */
  const userTableWrapRef = ref(null);
  ...
  /** 选择成员表格滚动事件 */
  const handleUserTableScroll = (e) => {
    checkUserTablePing(e.currentTarget);
  };

  /** 计算选择成员表格列固定状态 */
  const checkUserTablePing = (wrapEl) => {
...
<style lang="scss" scoped>
  .user-table-wrap {
    ...
    &.is-ping-left :deep(.user-table-index) {
      backdrop-filter: var(--pq-table-fixed-backdrop-filter);
      ...
      &::before {
        ...
        bottom: -1px;
        right: -10px;
        box-shadow: var(--pq-table-fixed-left-shadow);
    ...
    &.is-ping-right :deep(.user-table-action) {
      ...
      &::before {
        ...
        bottom: -1px;
        left: -10px;
        box-shadow: var(--pq-table-fixed-right-shadow);
        transition: box-shadow 0.2s;
        pointer-events: none;
      }
    }
