<!-- 皮肤背景 -->
<template>
  <div class="pq-setting-layout-list pq-setting-skin-list">
    <SettingLayoutItem
  ...
  <template v-if="showUserSkin">
    <SettingItem :title="t('layout.setting.skinConfig.wallpaper')">
      ...
      >
        <ElIcon
          class="pq-setting-skin-item-cover pq-setting-bg-upload"
...
<script setup>
  ...
  import { isColorValue } from 'pq-admin-ent/es/utils/theme-util';
          ...
          new Promise((resolve) => {
            document.documentElement.animate(
              { clipPath: ['inset(0 3px 0 0)', 'inset(0 100% 0 0)'] },
              {
                ...
                easing: 'ease-in',
                fill: 'forwards',
                pseudoPqment: '::view-transition-old(root)'
              }
            ).onfinish = () => resolve();
          }),
          new Promise((resolve) => {
            document.documentElement.animate(
              { clipPath: ['inset(0 0 0 100%)', 'inset(0 0 0 3px)'] },
              {
                ...
                easing: 'ease-in',
                fill: 'forwards',
                pseudoPqment: '::view-transition-new(root)'
              }
            ).onfinish = () => resolve();
          })
        ]);
