<!-- 常用布局 -->
<template>
  <div class="pq-setting-layout-item" :class="{ 'is-active': active }">
    <div class="pq-setting-layout-item-cover" :style="coverStyle">
      <template v-if="cover">
        <template v-if="typeof cover === 'string'">
          <component v-if="LAYOUT_COVERS[cover]" :is="LAYOUT_COVERS[cover]" />
          <div
            v-else
            class="pq-setting-layout-item-cover-img"
            :style="{
    ...
    <div class="pq-setting-layout-item-label">{{ title }}</div>
  </div>
</template>
